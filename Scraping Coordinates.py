...
url_sights = "https://www.planetware.com/tourist-attractions-/london-eng-l-lon.htm"
...
def clean_names_sight(x) : # cleaning first list scraped
    new_list = []
    for x in names_sights:
        new_list.append("".join(re.findall('\D+', x)))
    return new_list


# def get_coordinates(x): # Scraping coordinates of availables monuments 
#     ua = {"user-agent": "Mozilla/5.0"}
#     lat=[]
#     long = []
#     place = []
...
names_sights = [i.text for i in soup.select("h2.sitename")]
